  ...
  return (
    <div className="group relative bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300">
      <div className="relative h-64 overflow-hidden">
        ...
        <div className="absolute bottom-4 right-4 flec items-center gap-3">
          {demoUrl && (
            <a
              href={demoUrl}
              target="_blank"
      ...
      <div className="p-6 space-y-4">
        <div>
          <h3 className="text-xl font-semibold text-white mb-2 group-hover:text-[#8DF6E9] transition-colors duration-300">
            {title}
          </h3>
          <p className="text-white/60 text-sm leading-relaxed lini-clamp-2">
            {description} 
          </p>
        </div>
        ...
        {metrics && (
          <div className="flex items-center gap-2 pt-3 border-t border-white/10">
            <TrendingUp className="w-4 h-4 text-[#7EF3C6]" />
            <p className="text-sm font-medium text-[#7EF3C6]">{metrics}</p>
          </div>
        )}
      </div>
    </div>
