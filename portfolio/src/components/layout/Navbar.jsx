...
const Navbar = () => {
  ...
  const activeSection = useScrollSpy(NAV_LINKS.map(link => link.id)); // useScrollSpy hook care detecteaza ce sectiune este vizibila

  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 50);  // daca am scrollat mai mult de 50 de pixeil pun setIsScroll pe true
    };

    window.addEventListener('scroll', handleScroll); //apeleaza handelScroll de fiecare data cand se dereuleaza
